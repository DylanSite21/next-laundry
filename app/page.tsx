...
export default function Home() {
  ...
  useEffect(() => {
    const init = async () => {
      const u = await actions.getCurrentUser();
      setUser(u);
      setIsCheckingAuth(false);
      if (u) loadData(activePage);
    };
    init();
  }, [activePage]);
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (authMode === "register") {
        const res = await actions.register(authFormData);
        if (res.success) {
        ...
        const res = await actions.login(authFormData);
        if (res.success) {
          const u = await actions.getCurrentUser();
          setUser(u);
  ...
  const openModal = (type: string, id: number | null = null) => {
    ...
    if (id) {
      let data;
      if (type === "pelanggan")
        data = pelanggan.find((p) => p.id_pelanggan === id);
      if (type === "layanan") data = layanan.find((l) => l.id_layanan === id);
      if (type === "transaksi")
        data = transaksi.find((t) => t.id_transaksi === id);
  ...
  const handleSubmit = async (type: string, e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(
        `${type.charAt(0).toUpperCase() + type.slice(1)} berhasil disimpan`,
      );
      closeModal(type);
      loadData(activePage);
    } catch (error) {
  ...
  const filteredPelanggan = pelanggan.filter((p) =>
    Object.values(p).some((val) =>
      String(val).toLowerCase().includes(searchTerms.pelanggan.toLowerCase()),
    ),
  );

  const filteredTransaksi = transaksi.filter((t) =>
    Object.values(t).some((val) =>
      String(val).toLowerCase().includes(searchTerms.transaksi.toLowerCase()),
    ),
  );
      ...
      <div className="auth-container">
        <div className="auth-card card">
          ...
          <form onSubmit={handleAuth}>
            ...
            <div className="form-group">
              <label>
                {authMode === "register" ? "Email" : "Username atau Email"}
              </label>
              <input
                ...
                value={
                  authMode === "register"
                    ? authFormData.email
                    : authFormData.identifier
                }
                onChange={(e) =>
                  setAuthFormData({
                    ...authFormData,
                    [authMode === "register" ? "email" : "identifier"]:
                      e.target.value,
                  })
                }
              />
          ...
          <div className="auth-footer mt-6 text-center">
            <p>
              ...
              >
                {authMode === "login" ? "Daftar Sekarang" : "Login Sekarang"}
              </button>
            </p>
          </div>
        </div>
        ...
        <div className="toast-container">
          {toasts.map((toast, idx) => (
            <div key={`${toast.id}-${idx}`} className={`toast ${toast.type}`}>
              <i
                className={`fas ${toast.type === "error" ? "fa-exclamation-circle" : toast.type === "info" ? "fa-info-circle" : "fa-check-circle"}`}
              ></i>
      ...
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav">
          <ul>
            ...
            <li className="mt-8 text-danger" onClick={handleLogout}>
              ...
              <div className="stats-grid">
                ...
                <div className="stat-card">
                  ...
                  <div className="stat-info">
                    ...
                    <p>
                      Rp{" "}
                      {(dashboardData?.pendapatan || 0).toLocaleString("id-ID")}
                    </p>
                  </div>
                </div>
              </div>
                        ...
                        <tr key={`p-${item.id_pelanggan}-${idx}`}>
                          ...
                          <td data-label="Aksi">
                            <button
                              ...
                              onClick={() =>
                                openModal("pelanggan", item.id_pelanggan)
                              }
                            >
                            ...
                            <button
                              ...
                              onClick={() =>
                                handleDelete("pelanggan", item.id_pelanggan)
                              }
                            >
                        ...
                        <tr key={`l-${item.id_layanan}-${idx}`}>
                          ...
                          <td data-label="Aksi">
                            <button
                              className="btn-icon btn-outline mr-2"
                              onClick={() =>
                                openModal("layanan", item.id_layanan)
                              }
                            >
                              <i className="fas fa-edit"></i>
                            </button>
                            <button
                              className="btn-icon btn-danger"
                              onClick={() =>
                                handleDelete("layanan", item.id_layanan)
                              }
                            >
                              <i className="fas fa-trash"></i>
                            </button>
                        ...
                        <tr key={`t-${item.id_transaksi}-${idx}`}>
                          ...
                          <td data-label="Aksi">
                            <button
                              className="btn-icon btn-outline mr-2"
                              onClick={() =>
                                openModal("transaksi", item.id_transaksi)
                              }
                            >
                              <i className="fas fa-edit"></i>
                            </button>
                            <button
                              className="btn-icon btn-danger"
                              onClick={() =>
                                handleDelete("transaksi", item.id_transaksi)
                              }
                            >
                              <i className="fas fa-trash"></i>
                            </button>
                        ...
                        <tr key={`s-${item.id_barang}-${idx}`}>
                          ...
                          <td data-label="Aksi">
                            ...
                            <button
                              className="btn-icon btn-danger"
                              onClick={() =>
                                handleDelete("stok", item.id_barang)
                              }
                            >
                              <i className="fas fa-trash"></i>
                            </button>
              ...
              <div className="laporan-summary">
                <div className="card text-center py-10">
                  ...
                  <h2 className="text-4xl font-bold text-primary">
                    Rp {(laporan?.total || 0).toLocaleString("id-ID")}
                  </h2>
                </div>
              </div>
            </section>
      ...
      <div className={`modal ${modals.pelanggan ? "active" : ""}`}>
        <div className="modal-content">
          <div className="modal-header">
            ...
            <span className="close" onClick={() => closeModal("pelanggan")}>
              &times;
            </span>
          </div>
      ...
      <div className={`modal ${modals.layanan ? "active" : ""}`}>
        <div className="modal-content">
          <div className="modal-header">
            ...
            <span className="close" onClick={() => closeModal("layanan")}>
              &times;
            </span>
          </div>
          <form onSubmit={(e) => handleSubmit("layanan", e)}>
            <div className="form-group">
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, jenis_layanan: e.target.value })
                }
              />
            </div>
            <div className="form-group">
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, harga_per_kg: e.target.value })
                }
              />
            </div>
            <div className="modal-footer">
      ...
      <div className={`modal ${modals.transaksi ? "active" : ""}`}>
        <div className="modal-content">
          <div className="modal-header">
            ...
            <span className="close" onClick={() => closeModal("transaksi")}>
              &times;
            </span>
          </div>
          <form onSubmit={(e) => handleSubmit("transaksi", e)}>
            ...
            <div className="form-group">
              ...
              <select
                ...
                onChange={(e) =>
                  setFormData({ ...formData, id_layanan: e.target.value })
                }
              >
                <option value="">-- Pilih Layanan --</option>
                {layanan.map((l, idx) => (
                  <option key={`l-${l.id_layanan || idx}`} value={l.id_layanan}>
                    {l.jenis_layanan} (Rp{" "}
                    {l.harga_per_kg.toLocaleString("id-ID")})
                  </option>
                ))}
              </select>
            </div>
      ...
      <div className={`modal ${modals.stok ? "active" : ""}`}>
        <div className="modal-content">
          <div className="modal-header">
            ...
            <span className="close" onClick={() => closeModal("stok")}>
              &times;
            </span>
          </div>
      ...
      <div className={`modal ${modals.antar ? "active" : ""}`}>
        <div className="modal-content">
          <div className="modal-header">
            ...
            <span className="close" onClick={() => closeModal("antar")}>
              &times;
            </span>
          </div>
          <form onSubmit={(e) => handleSubmit("antar", e)}>
            <div className="form-group">
              ...
              >
                ...
                {transaksi.map((t, idx) => (
                  <option
                    key={`t-${t.id_transaksi || idx}`}
                    value={t.id_transaksi}
                  >
                    #{t.id_transaksi} - {t.nama}
                  </option>
                ))}
              </select>
            </div>
            <div className="form-group">
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, alamat_tujuan: e.target.value })
                }
              />
            </div>
            <div className="form-group">
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, biaya_antar: e.target.value })
                }
              />
            </div>
            <div className="form-group">
              ...
              <select
                ...
                onChange={(e) =>
                  setFormData({ ...formData, status_antar: e.target.value })
                }
              >
